...
namespace SL
{
    namespace Screen_Capture
    {
        ...
        inline void ExtractAndConvertToRGBA(const Image& img, char* dst)
        {
		...
		inline void ExtractAndConvertToRGB(const Image& img, char* dst)
		{
		...
		inline void ExtractAndConvertToRGB_3DS(const Image& img, char* dst, int w2 = 400, int h2 = 240)
		{
			auto imgsrc = StartSrc(img);
			auto imgdist = dst;

			double scaleWidth = (double)w2 / (double)Width(img);
			double scaleHeight = (double)h2 / (double)Height(img);

			int x2, y2;
			for (int y = 0;y<h2;y++) {
				for (int x = 0;x<w2;x++) {

					int pixel = (y * (w2 * 3)) + (x * 3);
					int nearestMatch = (((int)(y / scaleHeight) * (Width(img) * 4)) + ((int)(x / scaleWidth) * 4));

					*(imgdist + pixel) = *(imgsrc + nearestMatch);
					*(imgdist + pixel + 1) = *(imgsrc + nearestMatch + 1);
					*(imgdist + pixel + 2) = *(imgsrc + nearestMatch + 2);
				}
			}
		}

		inline int ExtractAndConvertToBGR(const Image& img, char* dst)
		{
			auto totalsize = Width(img) * 3 * Height(img);
			auto imgsrc = StartSrc(img);
			auto imgdist = dst;

			// check why it's null here ?
			if (imgdist == nullptr) return -1;
		...
		inline void ExtractAndConvertToBGR_3DS(const Image& img, char* dst, int w2 = 400, int h2 = 240)
		{
			auto imgsrc = StartSrc(img);
			auto imgdist = dst;
		...
		inline void ExtractAndConvertToRGB565(const Image& img, char* dst)
		{
			...
			for (auto h = 0; h < Height(img); h++) {
				for (auto w = 0; w < Width(img); w++) {
					unsigned short rgb = (*(imgsrc + 2) << 11) | (*(imgsrc + 1) << 5) | *(imgsrc);
					*imgdist++ = rgb;
					*imgdist++ = rgb << 8;
					imgsrc += 2;
				}
		...
		inline void ExtractAndConvertToRGB565_3DS(const Image& img, char* dst)
		{
			int w2 = 400;
			int h2 = 240;
			...
			for (int y = 0;y<h2;y++) {
				for (int x = 0;x<w2;x++) {
					...
					unsigned short rgb = *(imgsrc + nearestMatch + 2) << 11 | *(imgsrc + nearestMatch + 1) << 5 | *(imgsrc + nearestMatch);
					*imgdist++ = rgb;
					*imgdist++ = rgb << 8;
				}
			}
