...
void ScreenCaptureSession::onProcessUpdateThread(void* context)
{
	...
	while(!self->mIsStopEncode)
	{
		...
		//======================================================
		if (tsDif >= 0 ) {
			self->m_audioGrabber->GetAudioFrames();
...
void ScreenCaptureSession::serverUpdate()
{
	...
	if (m_clientSession->g_ss_isReceived) {
		if (m_staticPacketBuffer == nullptr) m_staticPacketBuffer = (u8*)malloc(TRANSFER_BUFFER_SIZE);
		int readSize = avio_read(mFormatContext->pb, m_staticPacketBuffer, TRANSFER_BUFFER_SIZE);
		if (readSize <= 0)
		{
			if(readSize < 0)
			{
		...
		{
			m_clientSession->PreparePacketAndSend(m_staticPacketBuffer, readSize);
		}
	}
}
...
void ScreenCaptureSession::writeVideoFrame( FrameData* frameData)
{
	...
	ret = avcodec_receive_packet(mVideoStream.CodecContext, mVideoStream.Packet);
	if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF)
	{
		std::cout << "Something must to be return here: VIDEO" << std::endl;
		av_packet_unref(mVideoStream.Packet);
		return;
	}
	else if (ret < 0)
	{
	...
	/*memcpy(mOutputStreamPool + mOutputStreamPoolSize, mVideoStream.Packet->data, mVideoStream.Packet->size);
	mOutputStreamPoolSize += mVideoStream.Packet->size;*/
	av_packet_unref(mVideoStream.Packet);
