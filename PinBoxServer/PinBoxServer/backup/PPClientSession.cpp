...
void PPClientSession::PreparePacketAndSend(u8 type, AVPacket* packet)
{
	g_ss_isReceived = false;
	u32 msgSize = packet->size + 17;
	//-------------------------------------------------------
	u8* buffer = (u8*)malloc(msgSize);
	u8* cursor = buffer;
	WRITE_U8(cursor, type)
	WRITE_U64(cursor, packet->pts);
	WRITE_U64(cursor, packet->dts);
	memcpy(cursor, packet->data, packet->size);
	//-------------------------------------------------------
	...
	sendMessageWithCodeAndData(MSG_CODE_REQUEST_NEW_SCREEN_FRAME, buffer, msgSize);
}
