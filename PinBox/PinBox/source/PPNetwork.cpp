...
bool PPNetwork::ppNetwork_processTmpBufferData()
{
	if (g_tmpReceivedBuffer != nullptr)
	{
		...
		if (g_receivedBuffer == nullptr)
			g_receivedBuffer = linearAlloc(g_waitForSize);
		...
		if(dataLeft > 0)
		{
			...
			void* tmpBuffer = g_tmpReceivedBuffer;
			void* g_tmpReceivedBuffer = linearAlloc(dataLeft);
			memcpy(g_tmpReceivedBuffer, tmpBuffer + pieceSize, dataLeft);
			g_tmpReceivedSize = dataLeft;
			linearFree(tmpBuffer);
			//--------------------------------------------------
			// if data left still > 0 that mean we finished current request
...
void PPNetwork::ppNetwork_listenToServer()
{
	...
	void* recvBuffer = linearAlloc(bufferSize);
	...
	}else
	{
		...
		if (isFullyReceived) {
			...
			if (dataLeft > 0)
			{
				...
				g_tmpReceivedBuffer = linearAlloc(dataLeft);
...
//----------------------------------------------------------
void PPNetwork::SendMessage(void *msgBuffer, u32 msgSize)
{
	if (g_connect_state == CONNECTED) {
